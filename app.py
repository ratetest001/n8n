...
def create_scene_video(scene, temp_dir, scene_index):
    ...
    # --- Build FFmpeg filter for ken burns + concat ---
    filter_parts = []
    inputs = []
    ...
    # Add audio input index
    audio_input_idx = num_images
    inputs.extend(['-i', audio_path])
    
    # Add text overlay (escape special characters)
...
def generate_video():
    try:
        data = request.json
        scenes = data  # array of 4 scene objects
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # For now return success - we'll add Drive upload next
